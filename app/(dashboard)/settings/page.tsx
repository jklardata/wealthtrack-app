...
export default function SettingsPage() {
  ...
  const handlePdfFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Upload PDF immediately for parsing
    ...
    try {
      ...
      const response = await fetch("/api/tax-returns/upload-pdf", {
        method: "POST",
        body: formData,
      });
      ...
      setTaxMessage({
        type: "success",
        text: `Successfully imported ${result.imported} tax return(s) from ${result.filename}`,
      });
      ...
      setTaxMessage({
        type: "error",
        text: error instanceof Error ? error.message : "Failed to parse PDF",
      });
    } finally {
    ...
    <div className="space-y-6 max-w-4xl">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Upload your TurboTax PDF directly — no terminal commands needed
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          <details className="bg-muted/30 rounded-lg border">
            <summary className="cursor-pointer p-4 font-medium hover:bg-muted/50 transition-colors">
          ...
          {/* Instructions (Collapsed by default since PDF upload is primary) */}
          <details className="bg-muted/30 rounded-lg border">
            <summary className="cursor-pointer p-4 font-medium hover:bg-muted/50 transition-colors">
            ...
            <div className="p-4 pt-0 space-y-4">
              ...
              <div className="bg-green-500/10 border border-green-500/20 rounded-lg p-3">
                <p className="text-sm text-green-700 dark:text-green-400">
                  <strong>Privacy:</strong> When running locally, the Python script runs entirely on your computer.
